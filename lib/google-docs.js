...
async function getAccessToken() {
  ...
  // Handle all possible formats Vercel might store the private key
  let privateKey = process.env.GOOGLE_PRIVATE_KEY || ''
  // Replace literal \n with real newlines
  privateKey = privateKey.replace(/\\n/g, '\n')
  // If key doesn't have proper header, it may be base64 encoded — decode it
  if (!privateKey.includes('-----BEGIN')) {
    try { privateKey = Buffer.from(privateKey, 'base64').toString('utf8') } catch (e) {}
  }
  // Final cleanup: normalize any remaining escaped newlines
  privateKey = privateKey.replace(/\\n/g, '\n').trim()
  ...
  try {
    ...
    throw new Error(`Failed to sign JWT — check GOOGLE_PRIVATE_KEY format. Details: ${signErr.message}`)
  }
