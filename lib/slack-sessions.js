/**
 * Mensaje de Slack para Sesiones de Carrera — #comercial.
 *
 ...
 *
 * Bot: Bondy Leads (separado de Rex para no acoplar flujos comerciales con
 * el sourcing de candidatos). Token en env var `SLACK_LEADS_BOT_TOKEN`.
 * Bondy Leads ya está en #comercial (canal privado). Si chat.postMessage
 * devuelve `not_in_channel`, hay que reinvitar al bot (`/invite @Bondy Leads`).
 * El POST igual no falla — el post a Slack es best-effort.
 */

const SLACK_BOT_TOKEN = process.env.SLACK_LEADS_BOT_TOKEN
const CHANNEL = process.env.SLACK_SESSIONS_CHANNEL_ID || 'CC7KPD9A9'
...
async function slackCall(method, payload) {
  if (!SLACK_BOT_TOKEN) {
    console.warn('[slack-sessions] SLACK_LEADS_BOT_TOKEN not set — skipping', method)
