...
export function renderWelcomeEmail({ unsubscribeUrl, preferences, audience = 'candidates' }) {
  const safeUnsub = escapeHtml(unsubscribeUrl)
  const prefs = prefsLine(preferences)
  const isRecruiters = audience === 'recruiters'
  const introLine = isRecruiters
    ? 'Acabás de suscribirte al digest semanal de Bondy con los roles nuevos de recruiting, talent y people ops en LATAM. Un solo mail, los lunes a las 10am ART, con lo que entró en los últimos 7 días.'
...
export function renderWelcomeEmailText({ unsubscribeUrl, preferences, audience = 'candidates' }) {
  const prefs = prefsLine(preferences)
  const isRecruiters = audience === 'recruiters'
  const intro = isRecruiters
    ? 'Acabás de suscribirte al digest semanal de Bondy con los roles nuevos de recruiting, talent y people ops en LATAM. Un solo mail, los lunes a las 10am ART, con lo que entró en los últimos 7 días.'
    : 'Acabás de suscribirte al digest semanal de Bondy con los roles tech nuevos en LATAM. Un solo mail, los lunes a las 10am ART, con lo que entró en los últimos 7 días.'
  return [
    'BONDY · BUSCO TRABAJO',
