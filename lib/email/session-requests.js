...
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background:#FEFCF9; padding:40px 20px;">
  <tr><td align="center">
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="560" style="max-width:560px; background:#FFFFFF; border:1px solid #E8E4DE;">
      ...
      ${bodyRows}
      ${consultora ? signatureHtml(consultora) : ''}
    </table>
...
// ─── Links de acción (mismos que los botones de Slack) ─────────────
const ACTION_HOST = process.env.TOOLS_BASE_URL || 'https://tools.wearebondy.com'
...
function actionUrl(id, action, consultora) {
  let u = `${ACTION_HOST}/api/session-requests/action?id=${encodeURIComponent(id)}&action=${action}&secret=${encodeURIComponent(ACTION_SECRET)}`
  if (consultora) u += `&consultora=${encodeURIComponent(consultora)}`
  return u
}

function actionBtn(label, url, danger) {
  const bg = danger ? '#FFFFFF' : '#4A8C40'
  const color = danger ? '#C0392B' : '#FFFFFF'
  const border = danger ? '#C0392B' : '#4A8C40'
  return `<a href="${escapeHtml(url)}" style="display:inline-block; background:${bg}; color:${color}; border:1px solid ${border}; font-family:'Courier New', monospace; font-size:11px; letter-spacing:1px; text-transform:uppercase; text-decoration:none; padding:11px 18px; margin:0 8px 8px 0;">${escapeHtml(label)}</a>`
}

// ─── 0 · NOTIFICACIÓN AL EQUIPO (hello@) ───────────────────────────
// Se manda en cada submit del form. Incluye los mismos links de acción
// que los botones de Slack, así el flujo funciona por email aunque Rex
// todavía no esté en #agentes-bondy.
export function renderSubmissionNotifyEmail({ row }) {
  const r = row
  const linkedin = r.linkedin_url
    ? `<a href="${escapeHtml(r.linkedin_url)}" style="color:#4A8C40;">${escapeHtml(r.linkedin_url)}</a>`
    : '—'
  const cv = r.cv_url
    ? `<a href="${escapeHtml(r.cv_url)}" style="color:#4A8C40;">${escapeHtml(r.cv_url)}</a>`
    : '—'

  let buttons
  if (r.consultora_preferida === 'La que tenga disponibilidad antes') {
    buttons =
      actionBtn('Aprobar · Mara', actionUrl(r.id, 'approve', 'Mara')) +
      actionBtn('Aprobar · Lucía', actionUrl(r.id, 'approve', 'Lucía')) +
      actionBtn('Rechazar', actionUrl(r.id, 'reject'), true)
  } else {
    buttons =
      actionBtn('Aprobar', actionUrl(r.id, 'approve', r.consultora_preferida)) +
      actionBtn('Rechazar', actionUrl(r.id, 'reject'), true)
  }

  const bodyRows = [
    para('Entró una solicitud nueva de <strong>Sesión de Carrera</strong>.', { pt: 28 }),
    block('Nombre', escapeHtml(r.nombre)),
    block('Email', escapeHtml(r.email)),
    block('LinkedIn · CV', `${linkedin}<br>${cv}`),
    block('Servicio · Consultora pedida', `${escapeHtml(r.producto)} &nbsp;·&nbsp; ${escapeHtml(r.consultora_preferida)}`),
    block('Origen', escapeHtml(r.pagina_origen)),
    block('Qué espera de las charlas', escapeHtml(r.expectativa || '—').replace(/\n/g, '<br>')),
    block('Mayor problema hoy', escapeHtml(r.problema || '—').replace(/\n/g, '<br>')),
    `<tr><td style="padding:26px 40px 0 40px;">${buttons}</td></tr>`,
    para('Aprobar o rechazar acá dispara el email automático al candidato. Los mismos botones están en el mensaje de #agentes-bondy.', { pt: 18, muted: true }),
  ].join('')

  return {
    subject: `Nueva solicitud de Sesión de Carrera — ${r.nombre}`,
    html: shell({
      preheader: `${r.nombre} pidió ${r.producto} · consultora: ${r.consultora_preferida}`,
      kicker: 'Nueva solicitud · Sesión de Carrera',
      bodyRows,
    }),
    text: [
      `Nueva solicitud de Sesión de Carrera`, '',
      `Nombre: ${r.nombre}`,
      `Email: ${r.email}`,
      `LinkedIn: ${r.linkedin_url || '—'}`,
      `CV: ${r.cv_url || '—'}`,
      `Servicio: ${r.producto}`,
      `Consultora pedida: ${r.consultora_preferida}`,
      `Origen: ${r.pagina_origen}`, '',
      `Qué espera: ${r.expectativa || '—'}`,
      `Mayor problema hoy: ${r.problema || '—'}`, '',
      r.consultora_preferida === 'La que tenga disponibilidad antes'
        ? `Aprobar (Mara): ${actionUrl(r.id, 'approve', 'Mara')}\nAprobar (Lucía): ${actionUrl(r.id, 'approve', 'Lucía')}`
        : `Aprobar: ${actionUrl(r.id, 'approve', r.consultora_preferida)}`,
      `Rechazar: ${actionUrl(r.id, 'reject')}`,
    ].join('\n'),
  }
}
