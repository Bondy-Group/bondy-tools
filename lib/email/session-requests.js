...
function signatureHtml(consultora) {
  const name = consultora === 'Lucía' ? 'Lucía Palomeque' : 'Mara Schmitman'
...
// ─── 1 · APROBACIÓN ────────────────────────────────────────────────
export function renderApprovalEmail({ nombre, producto, consultora, precio, paymentInstructions }) {
  const n = escapeHtml((nombre || '').split(' ')[0] || nombre)
  const precioLine = precio
    ? `Sale <strong>${escapeHtml(precio)}</strong> y se abona antes de la primera sesión.`
    : `El precio lo definimos en la primera charla según lo que necesites.`
  const bodyRows = [
    para(`Hola ${n},`, { pt: 28 }),
    para('Leímos tu solicitud y queremos avanzar.'),
    para(`Pediste <strong>${escapeHtml(producto)}</strong> con <strong>${escapeHtml(consultora)}</strong>. ${precioLine}`),
    para('Para reservar, primero el pago:'),
    block('Cómo pagar', escapeHtml(paymentInstructions).replace(/\n/g, '<br>')),
    para('Cuando confirmemos que entró, te llega el link para elegir día y horario en la agenda de ' + escapeHtml(consultora) + '. Suele ser dentro del día hábil.'),
  ...
  return {
    ...
    text: [
      ...
      `Pediste ${producto} con ${consultora}. ${precio ? `Sale ${precio} y se abona antes de la primera sesión.` : 'El precio lo definimos en la primera charla según lo que necesites.'}`, '',
      'Para reservar, primero el pago:', paymentInstructions, '',
      `Cuando confirmemos que entró, te llega el link para elegir día y horario en la agenda de ${consultora}. Suele ser dentro del día hábil.`, '',
