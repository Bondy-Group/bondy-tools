 ...
 */
export function renderWeeklyDigest({ roles, unsubscribeUrl, weekLabel }) {
...
<title>Bondy · Roles tech de la semana</title>
</head>
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td style="font-family:Georgia,'Times New Roman',serif;font-size:18px;font-weight:bold;color:#1A1A1A;letter-spacing:-0.01em;">
                    Bondy
                    <span style="font-family:'Courier New',monospace;font-size:10px;font-weight:normal;color:#7A7874;letter-spacing:0.18em;text-transform:uppercase;margin-left:8px;">Busco trabajo</span>
                  </td>
          ...
          <tr>
            <td style="padding:24px 0 8px;border-top:2px solid #1A1A1A;">
              <h1 style="font-family:Georgia,'Times New Roman',serif;font-size:30px;line-height:1.15;color:#1A1A1A;margin:0 0 12px;font-weight:700;">
                ${count} ${count === 1 ? 'rol' : 'roles'} tech ${count === 1 ? 'nuevo' : 'nuevos'}<br/>
          ...
          <tr>
            <td style="padding:32px 0 24px;text-align:center;">
              <a href="${HOST}/busco-trabajo?utm_source=bondy&utm_medium=email&utm_campaign=weekly-digest" style="font-family:'Courier New',monospace;font-size:12px;letter-spacing:0.16em;text-transform:uppercase;color:#FEFCF9;background:#1A1A1A;text-decoration:none;padding:14px 28px;display:inline-block;">
...
export function renderWeeklyDigestText({ roles, unsubscribeUrl, weekLabel }) {
  const count = roles.length
  const lines = [
    `BONDY · Busco trabajo — ${weekLabel || ''}`,
    '',
    `${count} ${count === 1 ? 'rol' : 'roles'} tech ${count === 1 ? 'nuevo' : 'nuevos'} esta semana.`,
  ...
  lines.push('─────────────────────────────')
  lines.push('')
  lines.push(`Ver todos: ${HOST}/busco-trabajo`)
