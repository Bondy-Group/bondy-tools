...
/**
 * Envía un email desde el Gmail del sender (crea draft y lo envía).
 * Funciona con scope gmail.compose. Permite definir displayName del From.
 */
export async function sendGmailEmail({ accessToken, to, subject, htmlBody, fromEmail, fromName }) {
  ...
  if (!draftRes.ok) {
    throw new Error('Gmail draft creation error: ' + (await draftRes.text()))
  }
  ...
  if (!sendRes.ok) {
    throw new Error('Gmail send error: ' + (await sendRes.text()))
  }
  return sendRes.json()
}
