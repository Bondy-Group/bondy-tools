...
// True if the role's "age" is within `hours`. Age is measured from publish date
// (true age) and, only when the source gives no publish date, from when we first
// saw it (created_at — stable, unlike collected_at which resets on every
// re-scrape). Used by the public board to show only recently-posted roles.
function isWithinAgeHours(row, hours, now) {
  const cutoff = now - hours * 3600000
  const pub = row.published_at ? new Date(row.published_at).getTime() : null
  const anchor = pub != null ? pub : (row.created_at ? new Date(row.created_at).getTime() : null)
...
export async function fetchOpenRoles({
  ...
  // FIX 2026-06-01: max "aging" for the public board. A role is shown only if
  // it was published (or, if no published date, first seen by us) within this
  // many hours. The board passes 72 → only roles posted in the last 3 days.
  maxAgeHours = null,
} = {}) {
  ...
  const params = new URLSearchParams({
    select:
      'id,title,company,location,countries,modality,seniority,tech_stack,category,source,source_url,min_salary,max_salary,currency,published_at,collected_at,created_at,status,language_required,geo_scope,is_remote',
  ...
  const roles = rows
    .map((r) => {
      ...
      // FIX 2026-06-01: max aging for the public board (e.g. 72h). Only show
      // roles posted recently (by published_at, or first-seen when absent).
      if (maxAgeHours != null && !isWithinAgeHours(r, maxAgeHours, now)) {
        return null
      }
