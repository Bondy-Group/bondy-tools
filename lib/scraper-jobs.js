...
export async function fetchRecruitingRoles({ days = 60, limit = 500 } = {}) {
  ...
  // PostgREST `or=` syntax. We pull in anything the scraper already tagged
  // as `category=hr` (most reliable signal) plus title `ilike` patterns to
  // catch edge cases (legacy rows, mis-classified jobs, sources where the
  // scraper can't infer category cleanly). False positives are excluded
  // client-side (cheaper than nesting `not.ilike`).
  const orFilter = [
    'category.eq.hr',
    'title.ilike.*recruit*',
    'title.ilike.*sourcer*',
  ...
  const roles = rows
    .map((r) => {
      ...
      const area = classifyRecruitingArea(r.title, r.category)
      if (!area) return null
