...
export async function fetchOpenRoles({ days = 60, limit = 500 } = {}) {
  ...
  const params = new URLSearchParams({
    select:
      'id,title,company,location,countries,modality,seniority,tech_stack,category,source,source_url,min_salary,max_salary,currency,published_at,collected_at,status',
  ...
  if (!res.ok) {
    ...
    return { roles: [], lastUpdate: null, newToday: 0 }
  }
  ...
  return { roles, lastUpdate, newToday }
}
