...
// TODAS las tablas que lee este panel (job_subscribers, newsletter_*,
// email_events) viven en el proyecto SCRAPER, igual que lib/subscribers.js.
// NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY apuntan al proyecto
// MAIN (ejvjpjwcbxicbknrbddb) y NO sirven para esta data.
const SUPABASE_URL = 'https://tchppyxhapxtjemxrbqm.supabase.co'
const SERVICE_ROLE = process.env.SCRAPER_SUPABASE_SERVICE_KEY || ''
...
async function fetchEmailEvents() {
  const url = `${SUPABASE_URL}/rest/v1/email_events?select=event_type,audience,campaign,recipient,subject,link,event_at,received_at&order=received_at.desc&limit=20000`
  const res = await fetch(url, { headers: headers(), cache: 'no-store' })
  if (!res.ok) {
  ...
  // Tracking por destinatario: { bucket → { email → {opened,clicked,...} } }
  const people = { tech: new Map(), hr: new Map(), newsletter: new Map(), otros: new Map(), all: new Map() }
  function touchPerson(map, r) {
    if (!r.recipient) return
    let p = map.get(r.recipient)
    if (!p) {
      p = {
        recipient: r.recipient,
        opened: false, clicked: false, bounced: false, complained: false,
        open_count: 0, click_count: 0,
        links: new Set(), last_open_at: null, last_click_at: null,
      }
      map.set(r.recipient, p)
    }
    const at = r.event_at || r.received_at
    if (r.event_type === 'email.opened') {
      p.opened = true; p.open_count++
      if (at && (!p.last_open_at || at > p.last_open_at)) p.last_open_at = at
    } else if (r.event_type === 'email.clicked') {
      p.clicked = true; p.click_count++
      if (r.link) p.links.add(r.link)
      if (at && (!p.last_click_at || at > p.last_click_at)) p.last_click_at = at
    } else if (r.event_type === 'email.bounced') {
      p.bounced = true
    } else if (r.event_type === 'email.complained') {
      p.complained = true
    }
  }
  for (const r of rows) {
    ...
    touchPerson(people[k], r)
    touchPerson(people.all, r)
    const at = r.event_at || r.received_at
    if (at) {
  ...
  // Map → arrays ordenados: primero los que clickearon, luego los que abrieron.
  function serializePeople(map) {
    return [...map.values()]
      .filter((p) => p.opened || p.clicked || p.bounced || p.complained)
      .map((p) => ({ ...p, links: [...p.links] }))
      .sort((a, b2) => {
        const score = (x) => (x.clicked ? 2 : 0) + (x.opened ? 1 : 0)
        const d = score(b2) - score(a)
        if (d !== 0) return d
        return (b2.last_click_at || b2.last_open_at || '').localeCompare(a.last_click_at || a.last_open_at || '')
      })
  }
  const engaged = {}
  for (const key of Object.keys(people)) engaged[key] = serializePeople(people[key])

  // ── #1 Candidatos calientes (cross-campaña, todos los públicos) ──────
  // Score de engagement: cada click pesa 3, cada apertura 1. Penaliza a
  // quien rebotó o marcó spam (no es un lead caliente aunque tenga opens
  // viejos). Es la señal de sourcing: gente activa y receptiva.
  const hot = [...people.all.values()]
    .map((p) => ({
      ...
      last_at: p.last_click_at || p.last_open_at || null,
      score: p.click_count * 3 + p.open_count - (p.bounced || p.complained ? 999 : 0),
    }))
    .filter((p) => p.score > 0)
    .sort((a, b2) => b2.score - a.score || (b2.last_at || '').localeCompare(a.last_at || ''))
    .slice(0, 50)

  // ── #2 Qué links se clickean más ────────────────────────────────────
  // clicks totales + clickers únicos por URL. Para un job digest = qué
  // roles realmente quiere la audiencia.
  const linkMap = new Map()
  for (const r of rows) {
    if (r.event_type !== 'email.clicked' || !r.link) continue
    let l = linkMap.get(r.link)
    if (!l) { l = { url: r.link, clicks: 0, clickers: new Set(), last_at: null }; linkMap.set(r.link, l) }
    l.clicks++
    if (r.recipient) l.clickers.add(r.recipient)
    const at = r.event_at || r.received_at
    if (at && (!l.last_at || at > l.last_at)) l.last_at = at
  }
  const topLinks = [...linkMap.values()]
    .map((l) => ({ url: l.url, clicks: l.clicks, unique_clickers: l.clickers.size, last_at: l.last_at }))
    .sort((a, b2) => b2.clicks - a.clicks || b2.unique_clickers - a.unique_clickers)
    .slice(0, 25)

  // ── #5 Lista de supresión (hard bounce / spam) ──────────────────────
  // Emails a sacar para proteger la reputación del dominio. Dedup por
  // recipient, marca el motivo (puede tener ambos).
  const supMap = new Map()
  for (const r of rows) {
    if (r.event_type !== 'email.bounced' && r.event_type !== 'email.complained') continue
    if (!r.recipient) continue
    let s = supMap.get(r.recipient)
    if (!s) { s = { recipient: r.recipient, bounced: false, complained: false, last_at: null }; supMap.set(r.recipient, s) }
    if (r.event_type === 'email.bounced') s.bounced = true
    else s.complained = true
    const at = r.event_at || r.received_at
    if (at && (!s.last_at || at > s.last_at)) s.last_at = at
  }
  const suppression = [...supMap.values()]
    .sort((a, b2) => (b2.last_at || '').localeCompare(a.last_at || ''))

  return { total_rows: rows.length, ...b, engaged, hot, topLinks, suppression }
}
