...
// Job board (bondy_roles + bondy_applications) lives in the SCRAPER Supabase project (tchppyxhapxtjemxrbqm),
// NOT in the main bondy-tools project (ejvjpjwcbxicbknrbddb). We use dedicated env vars to avoid clashing
// with the main project's SUPABASE_SERVICE_ROLE_KEY used by data-explorer, gmail_tokens, etc.
const SUPABASE_URL = (
...
export function supabaseHeaders() {
  if (!SERVICE_ROLE) {
    console.error('[lib/job-board] SUPABASE_SERVICE_ROLE_KEY is missing or empty in env')
  }
  return {
    apikey: SERVICE_ROLE,
...
export function serviceRolePresent() {
