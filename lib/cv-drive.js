...
async function getAccessToken() {
  ...
  if (!b64) throw new Error('Missing GOOGLE_SERVICE_ACCOUNT_JSON')
...
export async function uploadCvToDrive({ base64, filename }) {
  ...
  if (!res.ok || !data.id) {
    let saEmail = '?'
    try { saEmail = JSON.parse(Buffer.from(process.env.GOOGLE_SERVICE_ACCOUNT_JSON, 'base64').toString('utf8')).client_email } catch {}
