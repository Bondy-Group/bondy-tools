...
function parseServiceAccount() {
  ...
  // 3) Último recurso: extraer client_email y private_key por regex, por si el
  //    JSON está mal formado (private_key con saltos de línea literales).
  let txt = raw
  try {
    const dec = Buffer.from(raw, 'base64').toString('utf8')
    if (dec.includes('private_key')) txt = dec
  } catch {}
  const email = (txt.match(/"client_email"\s*:\s*"([^"]+)"/) || [])[1]
  let key = (txt.match(/"private_key"\s*:\s*"([\s\S]*?)"\s*[,}]/) || [])[1]
  if (email && key) {
    key = key.replace(/\\n/g, '\n')
    return { client_email: email, private_key: key }
  }
  throw new Error('GOOGLE_SERVICE_ACCOUNT_JSON no parseable (ni base64, ni JSON, ni regex)')
}
